...
const COCO_MOCK_RESPONSES: Record<string, string> = {
  ...
  "qa-011": "具体化妆室或机构我这边没有固定推荐名单，也不方便直接评价别人家，因为每个机构的定位、价格和擅长方向差别挺大的。你选的时候可以多留意几点：老师有没有系统认证、诊断体系是否完整、是不是用专业色布和工具、有没有真实案例可以参考。如果你在广州，可以来我们店里当面看看诊断流程，再决定适不适合你。",
};
...
function matchGeneralKnowledge(q: string): string | null {
  ...
  if (q.includes("口红") || q.includes("唇色") || q.includes("唇膏")) {
    return "口红/唇色主要看你的季型。春季型适合暖调珊瑚橘、暖调豆沙、番茄红、蜜桃粉；夏季型适合冷调玫瑰粉、豆沙粉、蜜桃乌龙；秋季型适合南瓜色、砖红、陶土色、焦糖色；冬季型适合冷调正红、玫红、酒红、浆果色。告诉我你的具体季型，我可以给更精准的方向。";
  }
