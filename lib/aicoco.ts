...
**资料不足或超出范围时**
- 明确说这块资料目前不够稳定，不能硬编，也不要胡编乱造。
- 说明还需要补充什么资料，或给出一个真实可行的方案/路径（比如"这个我需要看到你本人才能判断，建议你到店我们当面看"）。
- 不要为了显得完整而猜测。
...
const COCO_MOCK_RESPONSES: Record<string, string> = {
  ...
  "qa-004": "刚做完诊断有这种茫然太正常了，很多人都会卡在这一步。你不用急着一次性改完整个人，先从一个小切口开始。比如今天只做一件事：打开衣柜，把三件最不适合你季型的颜色挑出来，先放到一边。或者从一支适合你季型的口红开始换。动作越小，越容易启动。你想先试试哪个？",
