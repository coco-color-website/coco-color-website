import type { Metadata } from "next";
import TeacherAIChat from "@/app/components/teacher-ai-chat";

export const metadata: Metadata = {
  title: "老师 AI 分身 · Coco Color",
  description:
    "向 Coco Color 老师的 AI 分身提问，获取课程、色彩诊断、形象设计相关的专业解答。",
};

export default function TeacherAIPage() {
  return (
    <div className="min-h-screen bg-[var(--background)]">
      <header className="sticky top-0 z-50 border-b border-[var(--pink-soft)] bg-[var(--background)]/85 backdrop-blur">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3">
          <a
            href="/"
            className="text-sm font-medium text-[var(--pink-deep)] transition hover:opacity-80"
          >
            ← 返回首页
          </a>
          <span className="font-semibold tracking-wide">老师 AI 分身</span>
        </nav>
      </header>

      <main className="mx-auto max-w-6xl px-5 py-16">
        <div className="text-center">
          <h1 className="serif text-4xl uppercase text-[var(--pink-deep)] sm:text-5xl">
            Ask Teacher AI
          </h1>
          <p className="mt-3 text-[var(--foreground)]/70">
            选择一位老师，向 TA 的 AI 分身提问
          </p>
        </div>

        <div className="mt-12">
          <TeacherAIChat />
        </div>
      </main>
    </div>
  );
}
